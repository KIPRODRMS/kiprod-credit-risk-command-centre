...
export default function BoardOversightPage() {
  ...
  const analysis = useMemo(() => {
    ...
    const issues: BoardIssue[] = candidateActions.map((action) => {
      const record = recordMap.get(action.loan_account);
      const flags = (record?.risk_flags || []).join(" ").toLowerCase();
      ...
      if (action.escalation_level.includes("Level 4"))
      ...
      return {
        issueId: `BO-${action.action_id.replace(/^ACT-/, "")}`,
        ...
        latestNote:
          boardNotes.find((note) => note.issueId === `BO-${action.action_id.replace(/^ACT-/, "")}`)
            ?.note ||
          action.notes ||
          "No management note",
