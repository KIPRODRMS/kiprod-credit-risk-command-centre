...
const roles = ["KIPROD Admin", "Institution Admin", "Board Chair", "Board Member", "Board Secretary", "CEO", "Risk Manager", "Credit Manager", "Portfolio/Loans Manager", "Recovery Manager"];
const portalForRole: Record<string, string> = {
  "KIPROD Admin": "/admin-portal", "Institution Admin": "/admin-portal", "Board Chair": "/board-portal", "Board Member": "/board-portal", "Board Secretary": "/board-portal", CEO: "/ceo-portal", "Risk Manager": "/risk-manager-portal", "Credit Manager": "/credit-manager-portal", "Portfolio/Loans Manager": "/portfolio-manager-portal", "Recovery Manager": "/recovery-manager-portal",
};
const accessMatrix = [
  ...
  ["KIPROD Admin", "Platform support, diagnostics, configuration and audit", "Cannot erase accountability history"],
];
const portals = [
...
export default function AdminPortalPage() {
  ...
  useEffect(() => {
    ...
    loadMasterInstitutionProfile().then((result) => {
      ...
      if (existing.length) { setUsers(existing); return; }
      const initial: PortalUser[] = [{ id: crypto.randomUUID(), name: result.profile.primaryContact || "Institution Administrator", email: "", role: "Institution Admin", status: "Setup Required", portal: "/admin-portal", createdAt: new Date().toISOString() }];
  ...
  async function addUser() {
    ...
    const next: PortalUser = { id: crypto.randomUUID(), name: cleanName, email: cleanEmail, role, status: "Active", portal: portalForRole[role] || "/", createdAt: new Date().toISOString() };
    const updated = [...users, next];
    localStorage.setItem(STORAGE_KEY, JSON.stringify(updated)); setUsers(updated); setName(""); setEmail(""); setMessage(`${cleanName} was added as ${role}.`);
  ...
  async function toggleUser(user: PortalUser) {
    ...
    localStorage.setItem(STORAGE_KEY, JSON.stringify(updated)); setUsers(updated); setMessage(`${user.name} is now ${status.toLowerCase()}.`);
  ...
  return <main className="min-h-screen bg-[#eef2f6] p-4 text-slate-950 sm:p-8"><div id="dashboard" className="mx-auto max-w-[1500px] space-y-7">
    <header className="rounded-[2rem] bg-[#071426] p-8 text-white shadow-2xl"><p className="text-xs font-black uppercase tracking-[.22em] text-violet-300">Institution access and system control</p><h1 className="mt-3 text-3xl font-black sm:text-5xl">Command Centre System Administrator Dashboard</h1><p className="mt-4 max-w-3xl text-slate-300">{profile.institutionName || "Institution"}: user access, portal routing, institution configuration, system readiness and accountability evidence.</p><div className="mt-6 rounded-2xl border border-amber-300/30 bg-amber-300/10 p-4 text-sm text-amber-100"><strong>MVP security boundary:</strong> this registry simulates role access in browser storage. Production authentication, invitations and server-enforced permissions require the Supabase security phase.</div></header>
    ...
    <section id="users" className="scroll-mt-24 rounded-3xl bg-white p-6 shadow-sm"><div><p className="text-xs font-black uppercase tracking-[.18em] text-violet-700">Access administration</p><h2 className="mt-1 text-2xl font-black">Portal user registry</h2></div><div className="mt-5 grid gap-4 lg:grid-cols-[1fr_1fr_1fr_auto]"><label className="text-sm font-bold">Full name<input value={name} onChange={(event) => setName(event.target.value)} className="mt-2 w-full rounded-xl border p-3" placeholder="User's full name" /></label><label className="text-sm font-bold">Email address<input type="email" value={email} onChange={(event) => setEmail(event.target.value)} className="mt-2 w-full rounded-xl border p-3" placeholder="name@institution.org" /></label><label className="text-sm font-bold">Portal role<select value={role} onChange={(event) => setRole(event.target.value)} className="mt-2 w-full rounded-xl border p-3">{roles.map((item) => <option key={item}>{item}</option>)}</select></label><button type="button" onClick={() => void addUser()} className="self-end rounded-xl bg-violet-700 px-5 py-3 font-black text-white">Add user</button></div>
