...
function recommendedWatchAction(record: LoanRecord) {
  if (record.risk_status === "Amber") {
    return "Monitor closely and confirm next repayment commitment.";
  }
  ...
  return "No watchlist action required.";
}
...
export default function WatchlistPage() {
  ...
  const summary = useMemo(() => {
    const amber = watchlistRecords.filter(
      (record) => record.risk_status === "Amber"
    );

    const red = watchlistRecords.filter(
      (record) => record.risk_status === "Red"
    );

    const exposure = watchlistRecords.reduce(
      (sum, record) => sum + record.outstanding_balance,
      0
    );

    const arrears = watchlistRecords.reduce(
    ...
    return {
      ...
      redCount: red.length,
      totalWatchlist: watchlistRecords.length,
      exposure,
      arrears,
    };
  }, [watchlistRecords]);
        ...
        <div className="mb-8 flex flex-col justify-between gap-4 md:flex-row md:items-end">
          <div>
            ...
            <p className="mt-2 max-w-3xl text-slate-600">
              A focused management view of Amber and Red accounts that require
              early intervention before they become full NPLs.
            </p>
          </div>
        ...
        {records.length === 0 ? (
          ...
          <>
            <div className="grid gap-4 md:grid-cols-4">
              ...
              <div className="rounded-2xl bg-white p-5 shadow-sm">
                <p className="text-sm text-slate-500">Watchlist Exposure</p>
                <h2 className="mt-2 text-2xl font-bold text-slate-950">
            ...
            <div className="mt-6 rounded-2xl bg-white p-6 shadow-sm">
              ...
              <p className="mt-3 leading-7 text-slate-700">
                ...
                <strong>{summary.totalWatchlist}</strong> accounts with a total
                outstanding exposure of{" "}
                <strong>{formatKes(summary.exposure)}</strong> and arrears of{" "}
                <strong>{formatKes(summary.arrears)}</strong>. Red accounts
                should receive immediate management attention, while Amber
                accounts should be monitored closely to prevent deterioration.
              </p>
            </div>
              ...
              <div className="mt-5 overflow-x-auto">
                ...
                {watchlistRecords.length === 0 && (
                  <div className="rounded-xl bg-slate-100 p-6 text-center text-slate-600">
                    No Amber or Red accounts found in the current portfolio.
                  </div>
                )}
              </div>
            </div>
